...
export const BookingSection: React.FC<BookingSectionProps> = ({ onNavigate, initialService }) => {
      ...
      <div className="min-h-screen pt-24 pb-12 bg-primary-50 flex items-center justify-center">
        <div className="max-w-xl w-full mx-4">
          <div className="bg-white rounded-3xl p-12 shadow-2xl text-center border border-primary-100 animate-fade-in-up">
            ...
            <h3 className="text-3xl font-serif font-bold text-primary-900 mb-4">{t('booking.success')}</h3>
            <p className="text-primary-800 text-lg mb-8 leading-relaxed">
              {t('booking.successMessage').replace('{name}', formData.name)}
              <br/>
              {t('booking.confirmationEmail')} <span className="font-semibold text-primary-900">{formData.email}</span>.
            </p>
            ...
            <div className="bg-primary-50 rounded-2xl p-8 mb-8 text-left border border-primary-100 shadow-sm relative overflow-hidden group">
                ...
                <div className="relative z-10 grid grid-cols-1 sm:grid-cols-2 gap-8">
                  ...
                  <p className="font-serif font-bold text-primary-900 text-2xl leading-tight">{formData.service}</p>
                    </div>
                    <div>
            ...
            <div className="space-y-4">
              <button 
                onClick={() => {
                  setStatus('IDLE');
                  setFormData({ name: '', email: '', phone: '', date: '', time: '', service: '', honeypot: '' });
                }}
  ...
  const inputClasses = "w-full pl-12 pr-4 py-3.5 bg-white border border-primary-100 rounded-xl focus:bg-white focus:border-primary-500 focus:ring-4 focus:ring-primary-500/15 focus:shadow-md outline-none transition-all duration-200 text-primary-900 placeholder:text-primary-400";
  const selectClasses = "w-full pl-4 pr-10 py-3.5 bg-white border border-primary-100 rounded-xl focus:bg-white focus:border-primary-500 focus:ring-4 focus:ring-primary-500/15 focus:shadow-md outline-none transition-all duration-200 appearance-none cursor-pointer text-primary-900 placeholder:text-primary-400";
  const selectWithIconClasses = "w-full pl-12 pr-10 py-3.5 bg-white border border-primary-100 rounded-xl focus:bg-white focus:border-primary-500 focus:ring-4 focus:ring-primary-500/15 focus:shadow-md outline-none transition-all duration-200 appearance-none cursor-pointer text-primary-900";
            ...
            <div className="bg-white/10 backdrop-blur-md p-6 rounded-2xl border border-white/10">
                <div className="flex items-center gap-4 mb-4">
                  <div className="w-12 h-12 bg-primary-500 rounded-full flex items-center justify-center">
                    ...
                    <div>
                    <p className="text-sm text-primary-100 uppercase tracking-wider">{t('booking.directLine')}</p>
      ...
      <div className="lg:w-7/12 w-full flex-1 bg-white overflow-y-auto">
           <div className="max-w-2xl mx-auto px-6 py-12 lg:py-24">
            <div className="mb-10 lg:hidden">
              <button onClick={() => onNavigate && onNavigate('home')} className="flex items-center gap-2 text-primary-700 mb-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-8 animate-fade-in-up">
                ...
                <div className="space-y-6">
                    <h3 className="text-sm font-semibold text-primary-700 uppercase tracking-wider border-b border-primary-100 pb-2">{t('booking.personalDetails')}</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                            <label className="text-sm font-medium text-primary-900">{t('booking.name')} <span className="text-red-500">*</span></label>
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-medium text-primary-900">{t('booking.email')} <span className="text-red-500">*</span></label>
                            <div className="relative">
                        ...
                        <div className="space-y-2 md:col-span-2">
                            <label className="text-sm font-medium text-primary-900">{t('booking.phone')} <span className="text-red-500">*</span></label>
                            <div className="relative">
                ...
                <div className="space-y-6">
                    <h3 className="text-sm font-semibold text-primary-700 uppercase tracking-wider border-b border-primary-100 pb-2">{t('booking.appointmentDetails')}</h3>
                    <div className="space-y-6">
                        <div className="space-y-2">
                            <label className="text-sm font-medium text-primary-900">{t('booking.service')} <span className="text-red-500">*</span></label>
                            <div className="relative">
                                <select
                                    required
                                    name="service"
                                ...
                                <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 text-primary-400 pointer-events-none" size={18} />
                            </div>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                             <div className="space-y-2">
                                <label className="text-sm font-medium text-primary-900">{t('booking.date')} <span className="text-red-500">*</span></label>
                                <div className="relative">
                                    <Calendar className="absolute left-4 top-1/2 -translate-y-1/2 text-primary-400" size={18} />
                                    <input
                                        required
                                        type="date"
                                        name="date"
                             ...
                             <div className="space-y-2">
                                <label className="text-sm font-medium text-primary-900">{t('booking.time')} <span className="text-red-500">*</span></label>
                                <div className="relative">
                                    <Clock className="absolute left-4 top-1/2 -translate-y-1/2 text-primary-400" size={18} />
                                    <select
                                        required
                                        name="time"
                                        value={formData.time}
