      ...
      <div className="relative h-[65vh] overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 w-full p-8 md:p-16">
          <div className="max-w-7xl mx-auto">
            ...
            <h1 className="text-5xl md:text-7xl font-serif font-bold text-white mb-6 animate-fade-in-up drop-shadow-md">
              {t(`services.items.${service.id}.title`)}
            </h1>
            <p
              className="text-xl md:text-2xl text-slate-100 max-w-3xl font-light leading-relaxed animate-fade-in-up drop-shadow-sm"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-20">
          ...
          <div className="lg:col-span-2 space-y-16">
            ...
            <div className="animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
              <span className="text-primary-600 font-bold tracking-[0.2em] text-xs uppercase block mb-4">
                {t('services.overview')}
              </span>
            ...
            {/* Key Benefits */}
            <div
              className="bg-slate-50 p-10 rounded-3xl border border-slate-100 shadow-sm animate-fade-in-up"
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-8">
                {(tRaw<string[]>(`services.items.${service.id}.benefits`) || service.benefits).map(
                  (benefit, idx) => (
            ...
            <div className="animate-fade-in-up" style={{ animationDelay: '0.4s' }}>
              <h3 className="text-3xl md:text-4xl font-serif font-bold text-slate-900 mb-10">
              ...
              <div className="space-y-10 relative before:absolute before:inset-0 before:left-[19px] before:top-4 before:h-[calc(100%-24px)] before:w-0.5 before:bg-slate-200">
                {(
                  tRaw<Array<{ title: string; description: string }>>(
                    `services.items.${service.id}.process`
                  ) || service.process
                ).map((step, idx) => (
          ...
          <div className="lg:col-span-1 animate-fade-in-up" style={{ animationDelay: '0.5s' }}>
            <div className="sticky top-32 bg-slate-900 text-white p-8 rounded-3xl shadow-2xl ring-1 ring-white/10">
              ...
              <p className="text-slate-300 mb-8 leading-relaxed text-lg font-light">
                {t('services.bookConsultation').replace(
                  '{service}',
                  t(`services.items.${service.id}.title`)
                )}
              </p>
              ...
              <button
                onClick={() =>
                  onNavigate('booking', undefined, t(`services.items.${service.id}.title`))
                }
