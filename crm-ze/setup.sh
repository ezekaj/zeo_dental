...
echo "[1/16] Waiting for ManagerCRM to start..."
...
echo "[2/16] Setting application name to ManagerCRM..."
...
# Patch navbar link
echo "[4/16] Updating navbar brand link..."
...
echo "[5/16] Applying Z.E Digital Tech branding..."
...
echo "[6/16] Installing Guided Tour module..."
...
echo "[7/16] Injecting tour into main page..."
...
echo "[8/16] Importing Albanian translations..."
...
echo "[11/16] Configuring dental procedures and settings..."
...
echo "[12/16] Configuring email notifications..."
if [ -n "${SMTP_HOST:-}" ]; then
  docker exec "$CONTAINER" mysql -h crm-ze-db -u root -proot openemr -e "
...
echo "[14/16] Translating dental content to Albanian..."
...
# Fix Albanian UI translations (Provider→Doktor, OpenEMR→ManagerCRM, etc.)
echo "[15/16] Fixing Albanian UI translations..."
docker cp "$SCRIPT_DIR/translations/fix_albanian_ui.sql" "$CONTAINER":/tmp/fix_albanian_ui.sql
docker exec "$CONTAINER" sh -c "mysql -h crm-ze-db -u root -proot openemr < /tmp/fix_albanian_ui.sql"
docker exec "$CONTAINER" rm -f /tmp/fix_albanian_ui.sql
echo "  Provider→Doktor, OpenEMR→ManagerCRM, Widget→Miniaplikacion fixed!"

# Clear template caches
echo "[16/16] Clearing caches..."
